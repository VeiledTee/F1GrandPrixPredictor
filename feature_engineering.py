...
def _standardise_quali_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    drv = next((cols[k] for k in cols if k in {"driver", "driver_abbreviation", "abbr"}), None)
    lap = next((cols[k] for k in cols if k in
                {"fastest lap", "fastest_lap", "fastest_lap_time", "lap_time", "time"}), None)
    grd = next((cols[k] for k in cols if k in
                {"position", "grid_position", "start_grid"}), None)
    if drv is None or lap is None:
        raise ValueError("Quali CSV must contain at least driver and lap‑time columns")

    out = pd.DataFrame()
    out["driver"] = df[drv]
    out["best_quali_time"] = df[lap].apply(_time_to_seconds)
    if grd is not None:
...
def engineer_features(
    ...
    *,
    ...
    if external_quali_df is not None and not external_quali_df.empty:
        ...
        if season_year is None:
            ...
            if len(y_unique) == 1:
                ...
                raise ValueError("season_year must be provided (multiple seasons detected).")
        if gp_round is None:
            r_unique = df["round"].unique()
            if len(r_unique) == 1:
                gp_round = int(r_unique[0])
            else:
                raise ValueError("gp_round must be provided (multiple rounds detected).")
        ext["year"], ext["round"] = season_year, gp_round
        quali_df = pd.concat([quali_df, ext.assign(session_type="Q")], ignore_index=True)
    ...
    if "best_quali_time" in quali_df.columns:
        quali_summary = quali_df[["year", "round", "driver", "best_quali_time"]].copy()
    else:  # raw quali laps
    ...
    race_df["num_stops"] = race_df.groupby(["year", "round", "driver"])["compound_change"].transform("sum")
    ...
    compound_usage_frac.columns = [f"pct_{c.lower()}" for c in compound_usage_frac.columns]
    ...
    race_features["avg_stint_len"] = race_features["total_laps"] / (race_features["num_stops"] + 1)
    race_features.drop(columns="total_laps", inplace=True)

    # ------------------------------------------------------------------- #
    # 4. Merge all blocks
    # ------------------------------------------------------------------- #
    features = race_features.merge(quali_summary, on=["year", "round", "driver"], how="left")
    if not practice_summary.empty:
        features = features.merge(practice_summary, on=["year", "round", "driver"], how="left")

    features = (
        features.merge(grid_start, on=["year", "round", "driver"], how="left")
    ...
    features["season_avg_pos_delta"] = (
        features
        .groupby(["year", "driver"])["pos_change"]
        .transform("mean")
    )

    # drop sparse columns
    sparse_cols: Sequence[str] = features.columns[features.isna().mean() > MISSING_THRESHOLD]
...
def main():
    ...
    # mini_q = pd.read_csv("qualifying_china_2025.csv")
    # feats = engineer_features(raw, external_quali_df=mini_q, season_year=2025, gp_round=4)
